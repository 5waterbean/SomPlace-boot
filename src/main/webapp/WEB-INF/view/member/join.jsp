...
<body>
	...
	<div class="container">
		...
		<form:form modelAttribute="memberCommand" action="/member/join"
			onsubmit="return createMember(this)">
			<table>
				<tr>
					<th><label for="">아이디 💻</label></th>
					<td colspan="5"><input type="text" name="memberId"
						value="${memberCommand.memberId}"
						placeholder="4자 이상 15자 이내로 입력해주세요" pattern=".{4,15}" required
						title="4자 이상 15자 이내로 입력해주세요"></td>
					<td style="width: 25%;"><input type="button" id="idCheck"
						name="idCheck" value="중복확인" onClick="checkId()"></td>
				</tr>

				<tr>
					<th><label for="">비밀번호 🔒</label></th>
					<td colspan="5"><input type="password" name="pw"
						value="${memberCommand.pw}" placeholder="4자 이상 15자 이내로 입력해주세요"
						pattern=".{4,15}" required title="4자 이상 15자 이내로 입력해주세요"></td>
					<td><font color="red" size="2"><form:errors path="pw" /></font>
					</td>
				</tr>
				...
				<tr>
					...
					<td colspan="5"><input type="password" name="pwConfirm"
						value="${memberCommand.pwConfirm}" required></td>
					<td><font color="red" size="2"><form:errors
								path="pwConfirm" /></font></td>
				</tr>

				<tr>
					<th><label for="">이름 📰</label></th>
					<td colspan="5"><input type="text" name="name"
						value="${memberCommand.name}" required></td>
					<td><font color="red" size="2"><form:errors path="name" /></font>
					</td>
				</tr>

				<tr>
					<th><label for="">학번 🏫</label></th>
					<td colspan="5"><input type="text" name="studentNumber"
						value="${memberCommand.studentNumber}" placeholder="8자리로 작성해주세요"
						pattern=".{8,8}" required title="8자리로 작성해주세요"></td>
					<td><font color="red" size="2"><form:errors
								path="studentNumber" /></font></td>
				</tr>
				...
				<tr>
					...
					<td colspan="5"><select name="major">
							...
							<c:forEach var="major" items="${majorList}">
								<option value="${major}"
									<c:if test="${memberCommand.major == major}">selected</c:if>>${major}</option>
							</c:forEach>
					</select></td>
					<td><font color="red" size="2"><form:errors
								path="major" /></font></td>
				...
				<tr>
					...
					<td colspan="5"><input type="text" name="phone"
						value="${memberCommand.phone}" placeholder="- 빼고 숫자만 입력해주세요"
