...
public class MemberMeetingService {
	...
	public int updateMemberMeeting(int apply, int heart, String memberId, int meetingId) {
		
		if (apply == -1 && heart == 1) { // 모임 신청 (찜하기 되어있을 경우)
			return memberMeetingDao.updateApplyMeeting(memberId, meetingId);
		} else if (apply == 1 && heart == 1) { // 모임 신청 취소 (찜하기 되어 있을 경우)
			return memberMeetingDao.updateApplyCancelMeeting(memberId, meetingId);
		} else if (apply != -1 && heart == 0) { // 모임 찜하기 (신청, 수락 되어 있을 경우)
		...
		} else if (apply == 1 && heart == -1) { // 내가 생성한 모임 회원 수락 (신청이 되어있을 때)
